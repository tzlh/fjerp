 ...
 * 附件
 */
var settlement_bill_buy_file_data = [
 ...
 * 采购对账单数据
 */
var settlement_bill_buy_data = {"data":[
...
function settlement_bill_buy_clear_raw_data(contract_buy_contract_code_uuid) {
  $("#settlement_bill_buy_content" + contract_buy_contract_code_uuid).find(".settlement_bill_buy_box").html('<tr><td colspan="11" align="center">没数据</td></tr>');
}

/**
 * 服务器数据
 */
function settlement_bill_buy_server_data_cover(contract_code, contract_code_type, buy_type) {
...
function settlement_bill_buy_fill_variable_data(contract_buy_contract_code_uuid) {
  if(isJsonObjectHasData(settlement_bill_buy_data)) {
    var settlement_bill_buy_html = "";
    for (var i = 0; i < settlement_bill_buy_data.data.length; i++) {
      var settlement_bill_buy_actual_ullage = (settlement_bill_buy_data.data[i].load_quantity - settlement_bill_buy_data.data[i].unload_quantity)/settlement_bill_buy_data.data[i].load_quantity;
      var settlement_bill_buy_all_price = settlement_bill_buy_data.data[i].settle_quantity*settlement_bill_buy_data.data[i].goods_price;
      var settlement_bill_buy_returned = settlement_bill_buy_data.data[i].settle_quantity*settlement_bill_buy_data.data[i].goods_price - settlement_bill_buy_data.data[i].paid_amount;
      settlement_bill_buy_html +=
        '<tr>'+
          '<td>' + settlement_bill_buy_data.data[i].load_quantity + '</td>'+
          '<td>' + settlement_bill_buy_data.data[i].unload_quantity + '</td>'+
          '<td>' + settlement_bill_buy_data.data[i].contract_ullage + '‰</td>'+
          '<td>' + ((settlement_bill_buy_actual_ullage)*1000).toFixed(4) + '‰</td>'+
          '<td>' + settlement_bill_buy_data.data[i].settle_quantity + '</td>'+
          '<td>' + settlement_bill_buy_data.data[i].goods_price + '</td>'+
          '<td>' + settlement_bill_buy_data.data[i].invoice_quantity + '</td>'+
          '<td>' + settlement_bill_buy_all_price.toFixed(4) + '</td>'+
          '<td>' + settlement_bill_buy_data.data[i].paid_amount + '</td>'+
          '<td>' + settlement_bill_buy_returned.toFixed(4) + '</td>'+
          '<td>'+
            '<span class = "glyphicon glyphicon-info-sign settlement_bill_buy_ml15 settlement_bill_buy_modle_info" uuid = "' + settlement_bill_buy_data.data[i].uuid + '" contract_code = "' + settlement_bill_buy_data.data[i].contract_code + '" contract_code_uuid = "' + settlement_bill_buy_data.data[i].contract_code_uuid + '"></span>'+
            '<span class = "glyphicon glyphicon-pencil settlement_bill_buy_ml15 settlement_bill_buy_modle_pencil" uuid = "' + settlement_bill_buy_data.data[i].uuid + '" contract_code = "' + settlement_bill_buy_data.data[i].contract_code + '" contract_code_uuid = "' + settlement_bill_buy_data.data[i].contract_code_uuid + '"></span>'+
            '<span class = "glyphicon glyphicon-remove settlement_bill_buy_ml15 settlement_bill_buy_modle_remove" uuid = "' + settlement_bill_buy_data.data[i].uuid + '" contract_code = "' + settlement_bill_buy_data.data[i].contract_code + '" contract_code_uuid = "' + settlement_bill_buy_data.data[i].contract_code_uuid + '"></span>'+
          '</td>'+
        '</tr>';
    }
    //$("#settlement_bill_buy_"+trade_contract_code).find(".settlement_bill_buy_box")
    $("#settlement_bill_buy_content" + contract_buy_contract_code_uuid).find(".settlement_bill_buy_box").html(settlement_bill_buy_html);
  } else {
    //$("#settlement_bill_buy_"+trade_contract_code).find(".settlement_bill_buy_box")
    $("#settlement_bill_buy_content" + contract_buy_contract_code_uuid).find(".settlement_bill_buy_box").html('<tr><td colspan="11" align="center">没数据</td></tr>');
  }
...
function settlement_bill_buy_add_modle_func(obj) {
        ...
        '<div class = "modal-dialog" role = "document">'+
          '<div class = "modal-content">'+
            ...
            '<div class = "modal-footer" style = "text-align: center;">'+
              '<button type = "button" class = "btn btn-primary" id = "settlement_bill_buy_add_data_btn" contract_code = "' + contract_code + '" contract_code_uuid = "' + contract_code_uuid + '">添加</button>'+
...
function settlement_bill_buy_add_data_func(obj, contract_code_type, buy_type) {
  var contract_code = obj.attr("contract_code");
  var contract_code_uuid = obj.attr("contract_code_uuid");
  ...
  if ("1" == settlement_bill_buy__add_get_warehouse.status) {
    settlement_bill_buy_clear_raw_data(contract_code_uuid);
    settlement_bill_buy_server_data_cover(contract_code, "0", "1");
    settlement_bill_buy_fill_variable_data(contract_code_uuid); 
...
function settlement_bill_buy_edit_modle_func(obj) {
  ...
  if (0 < settlement_bill_buy_cluster_list.length) {
    ...
    settlement_bill_buy_file_data = settlement_bill_buy_file_arr;
    console.log(settlement_bill_buy_file_data);
  } else {
    settlement_bill_buy_file_data = [];
  }
    ...
    '<div class = "modal fade custom_modal" tabindex = "-1" id = "settlement_bill_buy_edit_modle_prop" role = "dialog" aria-labelledby = "myLargeModalLabel">'+
        '<div class = "modal-dialog" role = "document">'+
          '<div class = "modal-content">'+
            ...
            '</div>'+
            '<div class = "modal-footer" style = "text-align: center;">'+
              '<button type = "button" class = "btn btn-warning" id = "settlement_bill_buy_edit_data_btn" contract_code = "' + contract_code + '" contract_code_uuid = "' + contract_code_uuid + '" uuid  =  "' + uuid + '">修改</button>'+
              '<button type = "button" class = "btn btn-default" data-dismiss = "modal">取消</button>'+
            '</div>'+
          '</div>'+
        '</div>'+
  ...
  upload_attachment_edit_output("#settlement_bill_buy_edit_attch", settlement_bill_buy_file_data);
...
function settlement_bill_buy_edit_data_func(obj, contract_code_type, buy_type) {
  var uuid = obj.attr("uuid");
  var contract_code = obj.attr("contract_code");
  var contract_code_uuid = obj.attr("contract_code_uuid");
  var settlement_bill_buy_buyer_uuid = "";
...
function settlement_bill_buy_delete_modle_func(obj) {
  var uuid = obj.attr("uuid");
  var contract_code = obj.attr("contract_code");
  var contract_code_uuid = obj.attr("contract_code_uuid");
  var settlement_bill_buy_add_buyer_uuid = obj.attr("buyer_uuid");
  var settlement_bill_buy_add_seller_uuid = obj.attr("seller_uuid");
  var settlement_bill_buy_add_product_name = obj.attr("product_name");
    ...
    '<div class = "modal fade custom_modal" id = "settlement_bill_buy_delete_modle_prop" tabindex = "-1" role = "dialog">'+
      '<div class = "modal-dialog modal-sm" role = "document">'+
        '<div class = "modal-content">'+
          ...
          '<button type = "button" class = "btn btn-danger" id = "settlement_bill_buy_delete_modle_prop_btn"  uuid = "' + uuid + '" contract_code = "' + contract_code + '" contract_code_uuid = "' + contract_code_uuid + '">删除</button>'+
              '<button type = "button" class = "btn btn-default" data-dismiss = "modal">取消</button>'+
          '</div>'+
        '</div>'+
      '</div>'+
...
function settlement_bill_buy_delete_data_func(obj) {
  var uuid = obj.attr("uuid");
  var contract_code = obj.attr("contract_code");
  var contract_code_uuid = obj.attr("contract_code_uuid");
  var data = {
...
function settlement_bill_buy_info_modle_func(obj) {
  var uuid = obj.attr("uuid");
  var contract_code = obj.attr("contract_code");
  var contract_code_uuid = obj.attr("contract_code_uuid");
  var settlement_bill_buy_contract_ullage = "";
  ...
  var settlement_bill_buy_edit_html = 
    '<div class = "modal fade custom_modal" tabindex = "-1" id = "settlement_bill_buy_info_modle_prop" role = "dialog" aria-labelledby = "myLargeModalLabel">'+
      ...
      '</div>';
  $("body").append(settlement_bill_buy_edit_html);
  upload_attachment_preview_output("#settlement_bill_buy_info_attch", settlement_bill_buy_file_data);
