...
function report_test_fill_variable_data(vehicle_information_uuid) {
  if (isJsonObjectHasData(report_test_data)) {
    ...
    for (var i = 0; i < report_test_data.length; i++) {
      report_test_tbody += 
        '<tr>'+
          '<td>' + report_test_data[i].numerical_order + '</td>'+
          '<td>'+
            '<span class = "glyphicon glyphicon-info-sign report_test_ml15 report_test_detail" data-uuid = "' + report_test_data[i].uuid + '" data-vehicle_information_uuid = "' + report_test_data[i].vehicle_information_uuid + '"></span>'+
            '<span class = "glyphicon glyphicon-pencil report_test_ml15 report_test_edit" data-uuid = "' + report_test_data[i].uuid + '" data-vehicle_information_uuid = "' + report_test_data[i].vehicle_information_uuid + '"></span>'+
...
function report_test_server_data_cover(vehicle_information_uuid) {
  ...
  if (1 == report_test_get.status) {
    if (0 == report_test_get.count) {
      ...
      report_test_data = report_test_data_arr;
    }
  }
}
 ...
 * 获取化验单详情
 */
function report_test_get_letter(uuid) {
  console.log(uuid);
  var get_report_test_url = PROJECT_PATH + "lego/lego_fjTrade?servletName=getTestReport";
  var get_report_test_param_data = {};
  ...
  if (1 == report_test_get.status) {
    ...
    if (null != cluster_list_all) {
      ...
      report_test_file_data = report_test_file_arr;
    } else {
      report_test_file_data = {}
    }
  }
}
...
function report_test_add_data(vehicle_information_uuid) {
  ...
  add_report_test_param_data["vehicle_information_uuid"] = "45cd2b07a2d843a188522902083a696d";
  if("" != report_test_list) {
...
'     </div>';
  $("body").append(content);
  upload_attachment_edit_output("#report_test_edit_attch", report_test_file_data);
 ...
 * 化验单详情
 */
function report_test_detail_modal() {
  var content = 
...
'     </div>';
  $("body").append(content);
  upload_attachment_preview_output("#report_test_detail_attch", report_test_file_data);
 ...
 * 删除化验单
 */
function report_test_delete_modal(uuid, vehicle_information_uuid) {
  var content = 
...
function report_test_delete_data(uuid, vehicle_information_uuid) {
  ...
  delete_report_test_param_data["vehicle_information_uuid"] = vehicle_information_uuid;
