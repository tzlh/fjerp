...
var godown_entry_warehouse_pot_data = [
...
//    '</tr>';
    ...
    for (var i = 0; i < godown_entry_data.length; i++) {
      godown_entry_tbody += 
        '<tr>'+
          '<td>' + godown_entry_data[i].entry_datetime + '</td>'+
          '<td>' + godown_entry_data[i].tare_weight + '</td>'+
          '<td>' + godown_entry_data[i].gross_weight + '</td>'+
          '<td>' + godown_entry_data[i].net_weight + '</td>'+
          '<td>' + godown_entry_data[i].warehouse_pot_name + '</td>'+
          '<td>'+
            '<span class = "glyphicon glyphicon-info-sign godown_entry_ml15 godown_entry_detail" data-uuid = "' + godown_entry_data[i].uuid + '" data-vehicle_information_uuid = "' + godown_entry_data[i].vehicle_information_uuid + '"></span>'+
            '<span class = "glyphicon glyphicon-pencil godown_entry_ml15 godown_entry_edit" data-uuid = "' + godown_entry_data[i].uuid + '" data-vehicle_information_uuid = "' + godown_entry_data[i].vehicle_information_uuid + '"></span>'+
  ...
  if (1 == godown_entry_get.status) {
    if (0 == godown_entry_get.count) {
      ...
      for (var i = 0; i < result.length; i++) {
        ...
        net_weight_count = net_weight_count + net_weight_count_one;
    ...
    godown_entry_data = godown_entry_data_arr;
  }
}
 ...
 * 获取入库单详情
 */
function godown_entry_get_letter(uuid) {
  console.log(uuid);
  var get_godown_entry_url = PROJECT_PATH + "lego/lego_fjTrade?servletName=getGodownEntry";
  var get_godown_entry_param_data = {};
  ...
  if (1 == godown_entry_get.status) {
    ...
    current_godown_entry_data = {"entry_datetime": entry_datetime,"tare_weight": result[0].tare_weight,"gross_weight": result[0].gross_weight,"net_weight": result[0].net_weight,"uuid": result[0].uuid,"vehicle_information_uuid": result[0].vehicle_information_uuid,"warehouse_pot_uuid": result[0].warehouse_pot_uuid};
    ...
    if (null != cluster_list_all) {
      ...
      godown_entry_file_data = godown_entry_file_arr;
    } else {
      godown_entry_file_data = {}
    }
  }
}
 ...
 * 获取储罐
 */
function godown_entry_get_warehouse_pot() {
  var get_warehouse_pot_url = PROJECT_PATH + "lego/lego_fjTrade?servletName=getWarehousePot";
  var get_warehouse_pot_param_data = {};
  ...
  if (1 == godown_entry_get_warehouse_pot.status) {
    if (0 == godown_entry_get_warehouse_pot.count) {
      godown_entry_warehouse_pot_data = {};
    } else {
      ...
      godown_entry_warehouse_pot_data = warehouse_pot_arr;
    }
  } else {
    alert("获取库区失败");
    return;
 ...
 * 添加入库单
 */
function godown_entry_add_modal(vehicle_information_uuid) {
  var content = 
...
'     </div>';
  $("body").append(content);
  //储罐
  godown_entry_get_warehouse_pot();
  var workhouse_pot_select = '<option value = "">--请选择--</option>';
  for (var i = 0; i < godown_entry_warehouse_pot_data.length; i++) {
    workhouse_pot_select += '<option value = "' + godown_entry_warehouse_pot_data[i].warehouse_pot_uuid + '">' + godown_entry_warehouse_pot_data[i].warehouse_pot_name + '</option>'
...
function godown_entry_add_data(vehicle_information_uuid) {
  ...
  add_godown_entry_param_data["vehicle_information_uuid"] = vehicle_information_uuid;
 ...
 * 修改入库单
 */
function godown_entry_edit_modal(uuid, vehicle_information_uuid) {
  var content = 
...
'                       <label for = "">入库日期</label>'+
'                       <div class = "input-group">'+
'                         <input type = "text" class = "widget_datepicker form-control entry_datetime" value = "' + current_godown_entry_data.entry_datetime + '" aria-label = "Amount (to the nearest dollar)">'+
'                         <span class = "input-group-addon">'+
'                           <span class = "glyphicon glyphicon-calendar"></span>'+
'                         </span>'+
'                       </div>'+
...
'                       <label for = "">皮重</label>'+
'                       <div class = "input-group">'+
'                         <input type = "text" class = "form-control tare_weight" value = "' + current_godown_entry_data.tare_weight + '" aria-label = "Amount (to the nearest dollar)">'+
'                         <span class="input-group-addon">吨</span>'+
'                       </div>'+      
'                     </div>'+
'                   </form>'+
...
'                       <label for = "">毛重</label>'+
'                       <div class = "input-group">'+
'                         <input type = "text" class = "form-control gross_weight" value = "' + current_godown_entry_data.gross_weight + '" aria-label = "Amount (to the nearest dollar)">'+
'                         <span class="input-group-addon">吨</span>'+
'                       </div>'+      
'                     </div>'+
'                   </form>'+
...
'                       <label for = "">净重</label>'+
'                       <div class = "input-group">'+
'                         <input type = "text" class = "form-control net_weight" value = "' + current_godown_entry_data.net_weight + '" aria-label = "Amount (to the nearest dollar)">'+
'                         <span class="input-group-addon">吨</span>'+
'                       </div>'+      
'                     </div>'+
'                   </form>'+
...
'     </div>';
  ...
  for (var i = 0; i < $("#godown_entry_edit_modal select option").length; i++) {
    var warehouse_pot_uuid = current_godown_entry_data.warehouse_pot_uuid;
  ...
  upload_attachment_edit_output("#godown_entry_edit_attch", godown_entry_file_data);
...
function godown_entry_edit_data(uuid, vehicle_information_uuid) {
  ...
  edit_godown_entry_param_data["vehicle_information_uuid"] = vehicle_information_uuid;
 ...
 * 入库单详情
 */
function godown_entry_detail_modal() {
  var content = 
...
'                       <label for = "">入库日期</label>'+
'                       <div class = "input-group">'+
'                         <input type = "text" class = "widget_datepicker form-control entry_datetime" disabled = "disabled" value = "' + current_godown_entry_data.entry_datetime + '" aria-label = "Amount (to the nearest dollar)">'+
'                         <span class = "input-group-addon">'+
'                           <span class = "glyphicon glyphicon-calendar"></span>'+
'                         </span>'+
'                       </div>'+
...
'                       <label for = "">皮重</label>'+
'                       <div class = "input-group">'+
'                         <input type = "text" class = "form-control tare_weight" disabled = "disabled" value = "' + current_godown_entry_data.tare_weight + '" aria-label = "Amount (to the nearest dollar)">'+
'                         <span class="input-group-addon">吨</span>'+
'                       </div>'+      
'                     </div>'+
'                   </form>'+
...
'                       <label for = "">毛重</label>'+
'                       <div class = "input-group">'+
'                         <input type = "text" class = "form-control gross_weight" disabled = "disabled" value = "' + current_godown_entry_data.gross_weight + '" aria-label = "Amount (to the nearest dollar)">'+
'                         <span class="input-group-addon">吨</span>'+
'                       </div>'+      
'                     </div>'+
'                   </form>'+
...
'                       <label for = "">净重</label>'+
'                       <div class = "input-group">'+
'                         <input type = "text" class = "form-control net_weight" disabled = "disabled" value = "' + current_godown_entry_data.net_weight + '" aria-label = "Amount (to the nearest dollar)">'+
'                         <span class="input-group-addon">吨</span>'+
'                       </div>'+      
'                     </div>'+
'                   </form>'+
...
'     </div>';
  ...
  upload_attachment_preview_output("#godown_entry_detail_attch", godown_entry_file_data);
 ...
 * 删除入库单
 */
function godown_entry_delete_modal(uuid, vehicle_information_uuid) {
  var content = 
...
function godown_entry_delete_data(uuid, vehicle_information_uuid) {
  ...
  delete_godown_entry_param_data["vehicle_information_uuid"] = vehicle_information_uuid;
...
'    <div class = "panel-heading clearfix" id = "godown_entry_paid">入库单&nbsp;[净重总量&nbsp;:&nbsp;0]&nbsp;<span class = "glyphicon glyphicon-plus pull-right" data-vehicle_information_uuid = "45cd2b07a2d843a188522902083a696d" id = "godown_entry_add_modal_btn"></span></div>'+
