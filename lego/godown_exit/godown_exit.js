...
var godown_exit_plan_quantity_count = 0;
var godown_exit_actual_quantity_count = 0;
...
//    '</tr>';
    ...
    for (var i = 0; i < godown_exit_data.length; i++) {
      godown_exit_tbody += 
        '<tr>'+
          '<td>' + godown_exit_data[i].plan_quantity + '</td>'+
          '<td>' + godown_exit_data[i].actual_quantity + '</td>'+
          '<td>' + godown_exit_data[i].tare_weight + '</td>'+
          '<td>' + godown_exit_data[i].gross_weight + '</td>'+
          '<td>' + godown_exit_data[i].exit_datetime + '</td>'+
          '<td>'+
            '<span class = "glyphicon glyphicon-info-sign godown_exit_ml15 godown_exit_detail" data-uuid = "' + godown_exit_data[i].uuid + '" data-vehicle_information_uuid = "' + godown_exit_data[i].vehicle_information_uuid + '"></span>'+
            '<span class = "glyphicon glyphicon-pencil godown_exit_ml15 godown_exit_edit" data-uuid = "' + godown_exit_data[i].uuid + '" data-vehicle_information_uuid = "' + godown_exit_data[i].vehicle_information_uuid + '"></span>'+
...
function godown_exit_server_data_cover() {
  ...
  if (1 == godown_exit_get.status) {
    ...
    godown_exit_data = godown_exit_data_arr;
  }
}
 ...
 * 获取出库单详情
 */
function godown_exit_get_letter(uuid) {
  console.log(uuid);
  var get_godown_exit_url = PROJECT_PATH + "lego/lego_fjTrade?servletName=getGodownExit";
  var get_godown_exit_param_data = {};
  ...
  if (1 == godown_exit_get.status) {
    ...
    current_godown_exit_data = {"plan_quantity": result[0].plan_quantity,"actual_quantity": result[0].actual_quantity,"tare_weight": result[0].tare_weight,"gross_weight": result[0].gross_weight,"exit_datetime": exit_datetime,"uuid": result[0].uuid,"vehicle_information_uuid": result[0].vehicle_information_uuid};
    ...
    if (null != cluster_list_all) {
      ...
      for (var j = 0; j < cluster_list.length; j++) {
        var enterprise_management_get_godown_exit_file_url = PROJECT_PATH + "lego/lego_storage?servletName=getFileByClusterName";
        var enterprise_management_get_godown_exit_file_param_data = {};
        enterprise_management_get_godown_exit_file_param_data["cluster_name"] = cluster_list[j];
        var enterprise_management_get_godown_exit_file = ajax_assistant(enterprise_management_get_godown_exit_file_url, enterprise_management_get_godown_exit_file_param_data, false, true, false);
        console.log(enterprise_management_get_godown_exit_file);
        if (1 == enterprise_management_get_godown_exit_file.status) {
          var godown_exit_file_result = JSON.parse(enterprise_management_get_godown_exit_file.result);
      ...
      godown_exit_file_data = godown_exit_file_arr;
    } else {
      godown_exit_file_data = {};
    }
  }
}
 ...
 * 添加出库单
 */
function godown_exit_add_modal(vehicle_information_uuid) {
  var content = 
...
function godown_exit_add_data(vehicle_information_uuid) {
  ...
  add_godown_exit_param_data["vehicle_information_uuid"] = vehicle_information_uuid;
 ...
 * 修改出库单
 */
function godown_exit_edit_modal(uuid, vehicle_information_uuid) {
  var content = 
...
'                       <label for = "">计划量</label>'+
'                       <div class = "input-group">'+
'                         <input type = "text" class = "form-control plan_quantity" value = "' + current_godown_exit_data.plan_quantity + '" aria-label = "Amount (to the nearest dollar)">'+
'                         <span class="input-group-addon">吨</span>'+
'                       </div>'+      
'                     </div>'+
'                   </form>'+
...
'                       <label for = "">实发量</label>'+
'                       <div class = "input-group">'+
'                         <input type = "text" class = "form-control actual_quantity" value = "' + current_godown_exit_data.actual_quantity + '" aria-label = "Amount (to the nearest dollar)">'+
'                         <span class="input-group-addon">吨</span>'+
'                       </div>'+      
'                     </div>'+
'                   </form>'+
...
'                       <label for = "">皮重</label>'+
'                       <div class = "input-group">'+
'                         <input type = "text" class = "form-control tare_weight" value = "' + current_godown_exit_data.tare_weight + '" aria-label = "Amount (to the nearest dollar)">'+
'                         <span class="input-group-addon">吨</span>'+
'                       </div>'+      
'                     </div>'+
'                   </form>'+
...
'                       <label for = "">毛重</label>'+
'                       <div class = "input-group">'+
'                         <input type = "text" class = "form-control gross_weight" value = "' + current_godown_exit_data.gross_weight + '" aria-label = "Amount (to the nearest dollar)">'+
'                         <span class="input-group-addon">吨</span>'+
'                       </div>'+      
'                     </div>'+
'                   </form>'+
...
'                       <label for = "">出库日期</label>'+
'                       <div class = "input-group">'+
'                         <input type = "text" class = "widget_datepicker form-control exit_datetime" value = "' + current_godown_exit_data.exit_datetime + '" aria-label = "Amount (to the nearest dollar)">'+
'                         <span class = "input-group-addon">'+
'                           <span class = "glyphicon glyphicon-calendar"></span>'+
'                         </span>'+
'                       </div>'+
...
'     </div>';
  $("body").append(content);
  upload_attachment_edit_output("#godown_exit_edit_attch", godown_exit_file_data);
...
function godown_exit_edit_data(uuid, vehicle_information_uuid) {
  ...
  edit_godown_exit_param_data["vehicle_information_uuid"] = vehicle_information_uuid;
 ...
 * 出库单详情
 */
function godown_exit_detail_modal() {
  var content = 
...
'                       <label for = "">计划量</label>'+
'                       <div class = "input-group">'+
'                         <input type = "text" class = "form-control plan_quantity" disabled = "disabled" value = "' + current_godown_exit_data.plan_quantity + '" aria-label = "Amount (to the nearest dollar)">'+
'                         <span class="input-group-addon">吨</span>'+
'                       </div>'+      
'                     </div>'+
'                   </form>'+
...
'                       <label for = "">实发量</label>'+
'                       <div class = "input-group">'+
'                         <input type = "text" class = "form-control actual_quantity" disabled = "disabled" value = "' + current_godown_exit_data.actual_quantity + '" aria-label = "Amount (to the nearest dollar)">'+
'                         <span class="input-group-addon">吨</span>'+
'                       </div>'+      
'                     </div>'+
'                   </form>'+
...
'                       <label for = "">皮重</label>'+
'                       <div class = "input-group">'+
'                         <input type = "text" class = "form-control tare_weight" disabled = "disabled" value = "' + current_godown_exit_data.tare_weight + '" aria-label = "Amount (to the nearest dollar)">'+
'                         <span class="input-group-addon">吨</span>'+
'                       </div>'+      
'                     </div>'+
'                   </form>'+
...
'                       <label for = "">毛重</label>'+
'                       <div class = "input-group">'+
'                         <input type = "text" class = "form-control gross_weight" disabled = "disabled" value = "' + current_godown_exit_data.gross_weight + '" aria-label = "Amount (to the nearest dollar)">'+
'                         <span class="input-group-addon">吨</span>'+
'                       </div>'+      
'                     </div>'+
'                   </form>'+
...
'                       <label for = "">出库日期</label>'+
'                       <div class = "input-group">'+
'                         <input type = "text" class = "widget_datepicker form-control exit_datetime" disabled = "disabled" value = "' + current_godown_exit_data.exit_datetime + '" aria-label = "Amount (to the nearest dollar)">'+
'                         <span class = "input-group-addon">'+
'                           <span class = "glyphicon glyphicon-calendar"></span>'+
'                         </span>'+
'                       </div>'+
 ...
 * 删除出库单
 */
function godown_exit_delete_modal(uuid, vehicle_information_uuid) {
  var content = 
...
function godown_exit_delete_data(uuid, vehicle_information_uuid) {
  ...
  delete_godown_exit_param_data["vehicle_information_uuid"] = vehicle_information_uuid;
...
'    <div class = "panel-heading clearfix" id = "godown_exit_paid">出库单&nbsp;[净重总量&nbsp;:&nbsp;0]&nbsp;<span class = "glyphicon glyphicon-plus pull-right" data-vehicle_information_uuid = "45cd2b07a2d843a188522902083a696d" id = "godown_exit_add_modal_btn"></span></div>'+
